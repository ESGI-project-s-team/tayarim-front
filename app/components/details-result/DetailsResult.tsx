...
import {capturePaymentInFun, capturePaymentIntentInFun} from "@/app/components/details-result/actions";
...
const DetailsResult = () => {
        ...
        const createPaymentIntent = async () => {
            await capturePaymentIntentInFun(1000).then(
                (response) => {
        ...
        const handleSubmit = async (event: any) => {
            ...
            if (stripe && elements) {
                ...
                if (error) {
                    ...
                    console.log('Payment successful! Now you need to capture the payment.');
                    // Call your backend to capture the payment
                    return;
                    await capturePaymentInFun(paymentIntent.id).then(
                        (response) => {
                            console.log(response)
                            if (response.errors) {
                                setError(response.errors);
