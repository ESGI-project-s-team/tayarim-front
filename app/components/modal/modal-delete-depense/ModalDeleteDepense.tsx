...
import {deleteExpenseInFun} from "@/app/components/modal/modal-delete-depense/actions";

export default function ModalDeleteDepense({isOpen, onClose, id, getAllDepense}: {
    isOpen: boolean;
    onClose: () => void;
    getAllDepense: any;
    id: string;
    ...
    const handleDeleteDepense = async () => {
        setLoading(true)
        try {
            deleteExpenseInFun(id).then((response) => {
                if (response.errors) {
