...
export default function ModalEditOwner({ isOpen, onClose, owner }: { isOpen: boolean; onClose: () => void; owner: Owner }) {
    const focusElementRef = useRef<HTMLButtonElement | null>(null);
    const [formValues, setFormValues] = useState<Owner>({ ...owner }); // Initial state from `owner`
    ...
    const handleInputChange = (field: keyof Owner, value: string) => {
        setFormValues((prev) => ({ ...prev, [field]: value })); // Update the specific field
    };

    const handleActionUpdateOwner = async () => {
        try {
            await updateOwnerInFun(formValues); // Pass the updated form values
        } catch (error) {
                                    ...
                                    <div className="flex flex-col gap-9">
                                        <div className="rounded-sm border stroke-1 bg-white shadow">
                                            <div className="border-b border-[#dee4ee] py-4 flex justify-between px-7">
                                                <h3 className="font-medium text-black">Edit Owner</h3>
                                                <button onClick={onClose} className="text-[#3c50e0] font-medium">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-6 h-6 text-[#3c50e0]">
                                            ...
                                            <form action="#">
                                                <div className="p-7">
                                                    ...
                                                    <div className="mb-5">
                                                        <label className="mb-3 block text-sm font-medium text-black">Email</label>
                                                        <input
                                                    ...
                                                    <div className="mb-5">
                                                        <label className="mb-3 block text-sm font-medium text-black">Phone</label>
                                                        <input
                                                    ...
                                                    <div className="mb-5">
                                                        <label className="mb-3 block text-sm font-medium text-black">House</label>
                                                        <MultiSelectListbox />
                                                    </div>
